    ...
    tr.innerHTML = `
      ...
      <td class="px-6 py-3.5 text-right whitespace-nowrap">
        <div class="inline-flex items-center space-x-1">
          <button onclick="window.openViewModal('${user.user_id}')" class="text-slate-400 hover:text-brand-dark hover:bg-brand-light hover:border-brand-border/40 p-1.5 rounded-lg border border-transparent transition cursor-pointer" title="View Profile">
            <i class="fa-solid fa-eye text-xs"></i>
          </button>
          ${canEdit ? `
          <button onclick="window.openEditModal('${user.user_id}')" class="text-slate-400 hover:text-amber-600 hover:bg-amber-50 hover:border-amber-100 p-1.5 rounded-lg border border-transparent transition cursor-pointer" title="Edit Profile">
            <i class="fa-solid fa-pen text-xs"></i>
          </button>` : ''}
          <button onclick="window.openHistoryModal('${user.user_id}')" class="text-slate-400 hover:text-blue-600 hover:bg-blue-50 hover:border-blue-100 p-1.5 rounded-lg border border-transparent transition cursor-pointer" title="Security Audit Log">
            <i class="fa-solid fa-clock-rotate-left text-xs"></i>
          </button>
          ${canDelete ? `
          <button onclick="window.openArchiveUserModal('${user.user_id}')" class="text-slate-400 hover:text-amber-600 hover:bg-amber-50 hover:border-amber-100 p-1.5 rounded-lg border border-transparent transition cursor-pointer ${user.status === 'Archived' ? 'opacity-40 cursor-not-allowed' : ''}" ${user.status === 'Archived' ? 'disabled' : ''} title="Archive User Account">
